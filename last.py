...
ESP32_CAM_URLS = ["http://192.168.1.133/", "http://192.168.1.132/"]
...
def create_video_from_frames(date_str, part_number, camera_id):
    ...
    # Calculate the total number of frames and the desired frame rate
    total_frames = len(frame_files)
    desired_frame_rate = total_frames / 60.0  # 60 seconds

    cmd = [
        'ffmpeg', '-y', '-r', str(desired_frame_rate), '-f', 'concat', '-safe', '0', '-i', frame_list_file,
...
def log_event_to_firestore(event_type, event_time, camera_id, part_number, frame_url):
    try:
        date_str = event_time.split(" ")[0]  # Extract date from event_time
        doc_ref = firestore_db.collection('notifications').document(date_str).collection('all_notifications').document()
        doc_ref.set({
...
def log_event(event_type, date_str, part_number, stream_name, log_buffer, frame_path, camera_id):
    ...
    log_event_to_firestore(event_type, event_time, camera_id, part_number, frame_url)
